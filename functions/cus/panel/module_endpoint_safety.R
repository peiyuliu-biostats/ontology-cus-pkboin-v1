# overall safety endpoint setting
module_UI_safety_endpoint_setting <- function(id)
{
  ns <- NS(id)
  tagList(
    h5("Safety Endpoint Setting:"),
    # fixed anchor; endpoint blocks are inserted into / removed from here
    tags$div(id = ns("safe_endpoint_anchor"))
  )
}


module_server_safety_endpoint_setting <- function(input, output, session, all_rv)
{
  ns <- session$ns

  # remembers how many blocks are currently rendered and under which structure,
  # so we only add/remove the difference instead of rebuilding everything.
  ...
  safe_simu_model <- function(i) {
    tv <- all_rv$overall_setting$safe_type_vec
    if (!is.null(tv) && length(tv) >= i && !is.na(tv[i])) {
      if (tv[i] == "cont") all_rv$overall_setting$safe_cont_model
      else all_rv$overall_setting$safe_bin_model
    } else all_rv$overall_setting$safe_PK_model
  }

  # picks the per-endpoint UI / server builder by endpoint i's own resolved model.
  get_builder_i <- function(i) {
    simu  <- all_rv$overall_setting$simu_or_not
    if (simu == 1) {
      model <- safe_simu_model(i)
      if (model == 2) {
        list(ui = module_UI_inv_simu_Emax_safe_endpoint,
             server = module_server_inv_simu_Emax_safe_endpoint, kind = "E")
      } else {   # sigmoid (1) or continuous (3,4,5): same slope/intercept inputs
        list(ui = module_UI_inv_simu_sigmoid_safe_endpoint,
             server = module_server_inv_simu_sigmoid_safe_endpoint, kind = "S")
      }
    } else {
      model <- safe_simu_model(i)   # upload: resolved per endpoint from detected type
      if (model == 1) {
        list(ui = module_UI_inv_upload_sigmoid_safe_endpoint,
             server = module_server_inv_upload_sigmoid_safe_endpoint, kind = "uS")
      } else if (model == 2) {
        list(ui = module_UI_inv_upload_Emax_safe_endpoint,
             server = module_server_inv_upload_Emax_safe_endpoint, kind = "uE")
      } else {
        list(ui = module_UI_inv_upload_continuous_safe_endpoint,
             server = module_server_inv_upload_continuous_safe_endpoint, kind = "uC")
      }
    }
  }
  ...
  add_block <- function(i) {
    b <- get_builder_i(i)
    mod_id <- paste0("safe_endpoint_", b$kind, "_", i)
    insertUI(
      selector = paste0("#", ns("safe_endpoint_anchor")),
      where = "beforeEnd",
      ui = tags$div(
        id = ns(paste0("safe_block_", i)),  # wrapper id used by removeUI
        b$ui(ns(mod_id), i, safe_init(i))
      )
    )
  ...
  remove_block <- function(i) {
    removeUI(selector = paste0("#", ns(paste0("safe_block_", i))))
  }
  ...
  observe({
    ...
    all_rv$overall_setting$safe_PK_model
    all_rv$overall_setting$safe_type_vec
    all_rv$overall_setting$safe_cont_model
    all_rv$overall_setting$safe_bin_model
    req(!is.null(safe_num), !is.na(safe_num), safe_num >= 0)

    key <- safe_structure_key(safe_num)
    ...
    if (is.null(state$structure_key) || state$structure_key != key) {
      ...
      lapply(seq_len(safe_num), add_block)
      state$structure_key <- key
      state$rendered_n <- safe_num
      return()
    }

    # only the count changed: add or remove just the difference, leave the rest intact
    if (safe_num > state$rendered_n) {
      lapply(seq.int(state$rendered_n + 1, safe_num), add_block)
    } else if (safe_num < state$rendered_n) {
      lapply(seq.int(safe_num + 1, state$rendered_n), remove_block)
    }
    state$rendered_n <- safe_num
  })
}
