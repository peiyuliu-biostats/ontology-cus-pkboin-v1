module_UI_inv_utility_stepwise_enter_safe <- function(id, index)
{
  ns <- NS(id)
  tagList(
    ...
    fluidRow(
      column(width = 5,
             div(
               ...
               numericInput(ns("safe_knot_num"),
                            label = paste0("Number of Knots for Safety_V", index, " Stepwise Utility Function:"),
...
module_server_inv_utility_stepwise_enter_safe <- function(input, output, session, index, all_rv)
{
  ns <- session$ns

  # render the measurement/score rows from the current knot count.
  # depends ONLY on safe_knot_num (the row count); current measure/score values
  # are read with isolate() so editing a cell does NOT rebuild the rows.
  output$safe_rows <- renderUI({
    n <- all_rv$individual_utility_stepwise_setting$safe_knot_num[index]
    req(!is.null(n), !is.na(n), n %in% 1:20)
    isolate({
      meas <- all_rv$individual_utility_stepwise_setting$safe_measure[[index]]
      scor <- all_rv$individual_utility_stepwise_setting$safe_score[[index]]
  ...
  lapply(seq_len(20), function(i) {
    observeEvent(input[[paste0("safe_measure_", i)]], {
      req(i <= all_rv$individual_utility_stepwise_setting$safe_knot_num[index])
      tmp <- all_rv$individual_utility_stepwise_setting$safe_measure
      tmp[[index]][i] <- input[[paste0("safe_measure_", i)]]
      all_rv$individual_utility_stepwise_setting$safe_measure <- tmp
    }, ignoreNULL = TRUE, ignoreInit = TRUE)
    observeEvent(input[[paste0("safe_score_", i)]], {
      req(i <= all_rv$individual_utility_stepwise_setting$safe_knot_num[index])
      tmp <- all_rv$individual_utility_stepwise_setting$safe_score
      tmp[[index]][i] <- input[[paste0("safe_score_", i)]]
      all_rv$individual_utility_stepwise_setting$safe_score <- tmp
    }, ignoreNULL = TRUE, ignoreInit = TRUE)
  })
  ...
  observeEvent(debounce(reactive(input$safe_knot_num), 300)(), {
    ...
    if (warning$show_warning == TRUE) {
      ...
      all_rv$individual_utility_stepwise_setting$safe_knot_num[index] <- new_num
    }
  })
